% -*- plain-tex -*- 
% $Id: ref-card.tex,v 1.6 1993-02-16 04:09:42 amanda Exp $
% Reference Card for Auc TeX version 7.0
%**start of header
...
\key{Insert small caps text}{C-c C-f C-c}
...
\key{Format a paragraph}{C-c C-q C-p}
...
\key{(Un)comment region}{C-c ;}
\key{(Un)comment paragraph}{C-c \%}

\section{Outlining}

\key{Make all text invisible (excluding headings)}{C-c C-o C-t}
\key{Show all text}{C-c C-o C-a}
\key{Hide subtree}{C-c C-o C-h}
\key{Show subtree}{C-c C-o C-s}
\key{Show children (inventory)}{C-c C-o C-i}
\key{Hide entry}{C-c C-o C-e}
\key{Show entry}{C-c C-o C-l}

\section{Movement}

\key{Goto next visible heading}{C-c C-o C-n}
\key{Goto previous visible heading}{C-c C-o C-p}
\key{Goto next entry at same level}{C-c C-o C-f}
\key{Previous heading at same level}{C-c C-o C-b}
\key{Move up one level}{C-c C-o C-u}
\key{Up-list}{C-c \}}

\section{Miscellany}

\key{Normal mode}{C-c C-n}
\key{Describe mode}{C-c ?}
