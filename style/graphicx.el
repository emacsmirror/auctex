...
;;  (LaTeX2e).  If you want to use bb, angle or totalheight as arguments of
;;  includegraphics, set TeX-include-graphics-simple nil (default: t).
...
(defvar TeX-include-graphics-simple t
  "If nil, AUCTeX asks the following arguments: Bounding box (bb), Rotation
angle (angle), Total height (totalheight) in addition to the normal
arguments.")
  ...
  (let ((maybe-left-brace "[") (maybe-comma "")
    ...
    (when (not (zerop (length figwidth)))
      (insert maybe-left-brace maybe-comma
	      "width=" (car (TeX-string-divide-number-unit figwidth))
	      (if (zerop
		   (length
		    (car (cdr (TeX-string-divide-number-unit figwidth)))))
    ...
    (when (not (zerop (length figheight)))
      (insert maybe-left-brace maybe-comma
	      "height=" (car (TeX-string-divide-number-unit figheight))
	      (if (zerop
		   (length
		    (car (cdr (TeX-string-divide-number-unit figheight)))))
		  TeX-default-unit-for-image
		(car (cdr (TeX-string-divide-number-unit figheight)))))
      (setq maybe-comma ",")
      (setq maybe-left-brace ""))
