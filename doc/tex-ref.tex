...
\centerline{Copyright \copyright\ 1987, 2004 Free Software Foundation, Inc.}
\centerline{Copyright \copyright\ 1992 Kresten Krab Thorup}
\centerline{Copyright \copyright\ 1993 Per Abrahamsen}
...
Entering LaTeX mode calls the value of `{\tt text-mode-hook}', then
the value of `{\tt TeX-mode-hook}', and then the value of `{\tt
LaTeX-mode-hook}'.
...
\key{Pin a region}{C-c C-t C-r}
\key{Toggle usage of source specials}{C-c C-t C-s}
...
region (with \kbd{C-c C-r}) include the following (only for LaTeX
mode).

% \key{\TeX}{TeX}
% \overfullrule=0pt %The next line is too wide.
% \key{Run \TeX{} Interactively}{TeX Interactive}
% \key{Run PDF\TeX{}}{PDFTeX}
\key{Run LaTeX}{LaTeX}
\overfullrule=0pt
\key{Run LaTeX Interactively}{LaTeX Interactive}
\key{Run PDFLaTeX}{PDFLaTeX}
% \key{Run Con\TeX{}t (once)}{ConTeXt}
% \overfullrule=0pt
% \key{Run Con\TeX{}t Interactively (once)}{ConTeXt Interactive}
% \key{Run Con\TeX{}t Full}{ConTeXt Full}
% \key{Run Makeinfo}{Makeinfo}
% \overfullrule=0pt
% \key{Run Makeinfo with HTML output}{Makeinfo HTML}
